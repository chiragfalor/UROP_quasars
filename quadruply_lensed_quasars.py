...
class Conic():
  ...
  def coordinate_difference(self, c1, c2):
    '''
    input : 2 tuples (coordinates)
    output : 1 tuple (coordinate)
    Takes in 2 coordinates c1 and c2 and returns c1 - c2
    '''
    newc = (c1[0]- c2[0], c1[1] - c2[1])
    return newc
...
class NonCircularQuasar(Quasar):
  def __init__(self, coordinate_list):
    self.c1, self.c2, self.c3, self.c4 = coordinate_list
...
def AlignedEllipse_test():
  ...
  el = AlignedEllipse(S2P1, S2P2, S2P3, S2P4)
  print(el.parametric_angles)
  print(el.a, el.b)
  print(el.normalized_coordinates)
  NCQ = NonCircularQuasar(el.normalized_coordinates)
...
def print_central_flash_deviations():
  NCQlist = []
  ...
  for i in range(len(NCQlist)):
    ...
    print("\\hline")
#print_central_flash_deviations()
...
def central_flash_deviations():
  ...
  for i in range(len(NCQlist)):
    NCQ = NCQlist[i]
    deviations_list.append(NCQ.diff_theta_23*180/np.pi)
    #print(" "+str(i+2)+"{:.3f}".format(NCQ.diff_theta_23*180/np.pi))
  deviations_list = np.array(deviations_list)
  print(deviations_list)
  print("Mean deviation is ", np.mean(deviations_list))
  print("Standard deviations of deviation is ", np.std(deviations_list, dtype=np.float64))
