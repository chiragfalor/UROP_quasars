...
class RectangularHyperbola(Conic):
    ...
    def plot(self):
        ...
        def axes():
            ...
            plt.axvline(0, alpha=.1)
        axes()
        plt.contour(x, y,(a*x**2 + b*x*y + c*y**2 + d*x + e*y + f), [0], colors='k', zorder = -2)
...
class Quasar(Conic):
  ...
  def __repr__(self):
    return str(self.quasar_norm_array)
...
def Quasar_of_causticity_astroidal_angle(ca_list):
  '''
  takes in a list of tuples: (causticity, astroidal angle)
  returns a list of corresponding closest quasars via random search
  '''
  N = 400000
  random.seed(2)
  Quasar_list = []
  plotting_dictionary = dict.fromkeys(ca_list, 0)
  ...
  for i in range(N):
      ...
      try:
          ...
          print(t)
           ...
           print("bad err")
  with open("quasar_ca_2.txt", "wb") as fp:   #Pickling
    pickle.dump(Quasar_list, fp)
  return Quasar_list
#result = Quasar_of_causticity_astroidal_angle([(0.9, np.pi/4), (0.1, np.pi/4), (0.9, np.pi/2), (0.1, np.pi/2)])
